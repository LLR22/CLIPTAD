...
id_to_ChineseAction = {v: k for k, v in action_to_id.items()}

english_action_to_id = {translations[k]: v for k, v in action_to_id.items()}

english_to_chinese = {v :k for k, v in translations.items()}
# Output results
...
# 转换函数
def map_labels_to_actions(labels, id_to_action):
    mapped_labels = []
    for label in labels:
        action_id = label[1]  # 获取动作ID（如9, 10, 23...）
        action_text = id_to_action.get(action_id, f"unknown_{action_id}")  # 若未找到则标记为unknown
        mapped_label = label.copy()  # 避免修改原列表
        mapped_label[1] = action_text  # 替换ID为文本
