...
from model.TAD.head import PredictionHead, PredictionHeadLoc, PredictionHeadCls
...
class LocHead(nn.Module):
    def __init__(self, head_layer) :
        ...
        for i in range(head_layer):
            self.loc_heads.append(ScaleExp())
        self.PredictionHeadLoc = PredictionHeadLoc() # to change
        self.apply(init_weights)

    def forward(self, fpn_feats): 
        # fpn_feats: List[B * C * L]
        out_offsets = []
        ...
        for i, feat in enumerate(fpn_feats):
            ...
            # print(feat.shape)
            # assert False, 'test'
            loc_logits = self.PredictionHeadLoc(feat)
...
class ClsHead(nn.Module):
    def __init__(self,num_classes, head_layer, in_channel=512) :
        super().__init__()
        self.num_classes = num_classes
        self.PredictionHead = PredictionHeadCls()  # Assuming PredictionHead is defined elsewhere
        # Apply weight initialization to all submodules
        self.apply(init_weights)
    ...
    def forward(self, fpn_feats):
        ...
        for i, feat in enumerate(fpn_feats):
            assert not torch.isnan(feat).any(), "NaN detected in loc_logits before PredictionHead"
            conf_logits = self.PredictionHead(feat)
            # Apply the corresponding loc_head to the loc_logits
