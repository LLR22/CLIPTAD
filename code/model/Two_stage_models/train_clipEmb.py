...
from model.Two_stage_models.losses.loss import MultiSegmentLoss, LocLoss
...
class Trainer(object):
    def __init__(self,
        ...
        # loss setting -----------------------------------------------------------
        # self.loss = LocLoss(num_classes=config['model']['num_classes'], clip_length=config['dataset']['clip_length'])
    ...
    def _train_one_step(self, data, targets):
        ...
        try:
            sig_embed, loc, priors, logit_scale = self.model(data)
            
            # output_dict = self.model(data)

            # loc, priors = self.model(data)
        ...
        loc = loc.clamp(min=0)
        loss_l, loss_c = self.loss([sig_embed, loc, priors[0], logit_scale], targets)
        ...
        # loc_p = loc.clamp(min = 0)
        # loss_l = self.loss([loc_p, priors[0]], targets)
        ...
        # 无需分布式同步，直接返回损失
        #return loss.item(), loss_l.item(), loss_l.item()
...
if __name__ == '__main__':
    ...
    # path_checkpoint = '/home/yanrui/code/CLIPBased_TAD/model/CLIPTAD/preTrain_Emb/Two_stage_TAD-epoch-9.pt'
    path_checkpoint = '/home/yanrui/code/CLIPBased_TAD/result/Two_stage_TAD-epoch-79.pt'
